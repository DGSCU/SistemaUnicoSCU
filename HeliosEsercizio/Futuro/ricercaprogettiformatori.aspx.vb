Imports System.IO
Public Class ricercaprogettiformatori
    ...
    Private Sub LoadRiceraVolontari()
        ...
        Dim strNomeStore As String = "[SP_RICERCA_DOCUMENTI_FORMATORI]"
    ...
    Protected Sub cmdRicerca_Click(sender As Object, e As EventArgs) Handles cmdRicerca.Click
        dgProgetti.CurrentPageIndex = 0
        LoadRiceraVolontari()
        If dgProgetti.Items.Count = 0 Then
            lblmessaggio.Text = "Nessun Dato estratto."
            CmdEsporta.Visible = False
        Else
            lblmessaggio.Text = "Risultato Ricerca Progetti."
            dgProgetti.Visible = True
            CmdEsporta.Visible = True
        End If
    End Sub

    Private Sub dgProgetti_ItemCommand(source As Object, e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles dgProgetti.ItemCommand
        Select Case e.CommandName
            Case "Seleziona"
                'Response.Redirect("Presenze.aspx?IdEntita=" & dgProgetti.SelectedItem.Cells(0).Text)
            Case "Consulta"
                If (Session("TipoUtente") = "U" Or Session("TipoUtente") = "R") Then
                    Session("IdEnte") = e.Item.Cells(5).Text
                    Session("Denominazione") = e.Item.Cells(6).Text
                End If
                Response.Redirect("WfrmVisualizzaElencoDocumentiFormatori.aspx?VengoDa=RicercaProgettiFormatori&IdProg=" & e.Item.Cells(0).Text)
        End Select


    End Sub

    Private Sub dgProgetti_PageIndexChanged(source As Object, e As System.Web.UI.WebControls.DataGridPageChangedEventArgs) Handles dgProgetti.PageIndexChanged
        dgProgetti.CurrentPageIndex = e.NewPageIndex
        dgProgetti.DataSource = Session("appDtsRisRicerca")
        dgProgetti.DataBind()
    ...
    Private Sub CreaStampaCSV(ByVal MyDataSet As DataSet)
        ...
        NomeColonne(1) = "Titolo Progetto"
        NomeColonne(2) = "Totale Documenti"


        NomiCampiColonne(0) = "CodiceProgetto"
        NomiCampiColonne(1) = "TitoloProgetto"
        NomiCampiColonne(2) = "totaledocumentipresenti"

        'carico un datatable che userò poi nella pagina di stampa
        'il numero delle colonne è a base 0
        Session("DtbRicerca") = ClsServer.CaricaDataTablePerStampa(MyDataSet, 2, NomeColonne, NomiCampiColonne)
    End Sub
    ...
    Private Sub CaricaComboTipoDocumenti()
        ...
        strSql = "SELECT idPrefisso as idPrefisso, Prefisso as Prefisso, ordine   FROM  PrefissiEntitàDocumenti where tipoinserimento=3 UNION Select '', '',0 order by Ordine"
