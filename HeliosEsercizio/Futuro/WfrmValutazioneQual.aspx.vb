...
Public Class WfrmValutazioneQual
...
#End Region
    ...
    Private Sub PersonalizzaMaschera()
        txtDefCopertura.ReadOnly = True
        txtdefInflazione.ReadOnly = True
        txtPuntoCoerenza.ReadOnly = True
        txtPuntoRilevanza.ReadOnly = True
        cmdConferma.Visible = False
        cmdSalva.Visible = False
        Txt24EsteroAssenti.ReadOnly = True
        Txt24Assenti.ReadOnly = True
        Txt24Presenti.ReadOnly = True
        Txt25Assenti.ReadOnly = True
        Txt28Assenti.ReadOnly = True
        Txt29Assenti.ReadOnly = True
    ...
    Private Sub CreaTabellaParametri(ByVal sezione As String, ByRef contenitore As Panel)
        ...
        If dtrgenerico("NazioneBase") = True Then
            ...
            lbl28.Visible = True
            lbl29.Visible = True
            lbl31A.Visible = False
            lbl31P.Visible = False
            lbl32.Visible = False
            lbl35.Visible = False
            lbl36.Visible = False
            lbl24EsteroAssenti.Visible = False
            Txt24EsteroAssenti.Visible = False
        Else
            lblNazione.Text = "Estera"
            txtNazionale.Value = "2"
            lbl24EsteroAssenti.Visible = True
            Txt24EsteroAssenti.Visible = True
            lbl24A.Visible = False
            lbl24P.Visible = False
            lbl25.Visible = False
            lbl28.Visible = False
            lbl29.Visible = False
            lbl31A.Visible = True
    ...
    Private Sub cmdTrovaprogetto_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdTrovaprogetto.Click
        strsql = "Select statiattività.idstatoattività,statiattività.statoattività  from attività " & _
    ...
    Private Sub Salva(Optional ByVal clientIdSender As String = SENDER_PULSANTE_CONFERMA)
        ...
        Else
            ...
            strsql = "Insert into storicoProgetti (idprogetto,dataInserimento," & _
                    ...
                    "(" & hf_IdProgetto.Value & ",getdate()," & txtPuntoRilevanza.Text & "," & txtPuntoCoerenza.Text & "," & IIf(ddlinternet.SelectedValue.ToString = "", "null", ddlinternet.SelectedValue) & "," & _
                    "" & txtDefCopertura.Text & "," & txtdefInflazione.Text & "," & hf_StatoProgetto.Value & "," & _
                    "'" & Session("Utente") & "', '" & Replace(txtnote.Text, "'", "''") & "'," & txtPuntRegio.Text & ", "

            If Trim(Txt24Assenti.Text) = "" Then
                strsql = strsql & " '',"
            Else
                strsql = strsql & "'" & Replace(Trim(Txt24Assenti.Text), "'", "''") & "',"
            End If
            If Trim(Txt24Presenti.Text) = "" Then
                strsql = strsql & " '',"
            Else
                strsql = strsql & "'" & Replace(Trim(Txt24Presenti.Text), "'", "''") & "',"
            End If
            If Trim(Txt24EsteroAssenti.Text) = "" Then
                strsql = strsql & " '',"
            Else
                strsql = strsql & "'" & Replace(Trim(Txt24EsteroAssenti.Text), "'", "''") & "',"
            End If
            If Trim(Txt25Assenti.Text) = "" Then
                strsql = strsql & " '',"
            Else
                strsql = strsql & "'" & Replace(Trim(Txt25Assenti.Text), "'", "''") & "',"
            End If
            If Trim(Txt28Assenti.Text) = "" Then
                strsql = strsql & " '',"
            Else
                strsql = strsql & "'" & Replace(Trim(Txt28Assenti.Text), "'", "''") & "',"
            End If
            If Trim(Txt29Assenti.Text) = "" Then
                strsql = strsql & " '' ,"
            Else
                strsql = strsql & "'" & Replace(Trim(Txt29Assenti.Text), "'", "''") & "',"
            End If
            strsql = strsql & "" & txtDefSanzione.Text & "," & txtDefInfortuni.Text & ""
            strsql = strsql & ")"
            ChiudiDataReader(dtrgenerico)
        ...
        For k = 0 To dtDomanda.Rows.Count - 1
            ...
            For w = 0 To dtRisposta.Rows.Count - 1
                ...
                For s = 1 To j
                    ...
                    radio = TryCast(panelCaratteristicheProgetto.FindControl(idCheck), RadioButton)
                    If IsNothing(radio) Then
                        check = TryCast(panelCaratteristicheProgetto.FindControl(idCheck), CheckBox)
                        If Not IsNothing(check) Then
                    ...
                    If Not IsNothing(IdCampo) Then
                        If (checked) Then
                            If Mid(IdCampo, InStr(IdCampo, "W") + 1, Len(IdCampo)) = True Then
                                ...
                                End If
                                If lblNazione.Text = "Italiana" Then
                                    Select Case Campotxt
                                        Case Txt24Assenti.ID
                                            If (Txt24Assenti.Text = String.Empty) Then
                                                warningNote.AppendLine("Domanda 24 Punteggio 0. ")
                                                warningNote.AppendLine("<br\>")
                                                warningAttivo = True
                                            End If
                                        Case Txt24Presenti.ID
                                            If (Txt24Assenti.Text = String.Empty) Then
                                                warningNote.AppendLine("Domanda 24 Punteggio 1. ")
                                                warningNote.AppendLine("<br\>")
                                                warningAttivo = True
                                            End If
                                        Case Txt25Assenti.ID
                                            If (Txt25Assenti.Text = String.Empty) Then
                                                warningNote.AppendLine("Domanda 25 Punteggio 0. ")
                                                warningNote.AppendLine("<br\>")
                                                warningAttivo = True
                                            End If
                                        Case Txt28Assenti.ID
                                            If (Txt28Assenti.Text = String.Empty) Then
                                                warningNote.AppendLine("Domanda 28 Punteggio 0. ")
                                                warningNote.AppendLine("<br\>")
                                                warningAttivo = True
                                            End If
                                        Case Txt29Assenti.ID
                                            If (Txt28Assenti.Text = String.Empty) Then
                                ...
                                Else
                                    ...
                                    Select Case Campotxt
                                        Case Txt24Assenti.ID
                                            If (Txt24Assenti.Text = String.Empty) Then
                                                warningNote.AppendLine("Domanda 31 Punteggio 0. ")
                                                warningNote.AppendLine("<br\>")
                                                warningAttivo = True
                                            End If
                                        Case Txt24Presenti.ID
                                            If (Txt24Presenti.Text = String.Empty) Then
                                                warningNote.AppendLine("Domanda 31 Punteggio 1. ")
                                                warningNote.AppendLine("<br\>")
                                                warningAttivo = True
                                            End If
                                        Case Txt24EsteroAssenti.ID
                                            If (Txt24Presenti.Text = String.Empty) Then
                                                warningNote.AppendLine("Domanda 24 Punteggio 0(Estero). ")
                                                warningNote.AppendLine("<br\>")
                                                warningAttivo = True
                                            End If
                                        Case Txt25Assenti.ID
                                        ...
                                        Case Txt28Assenti.ID
                                            ...
                                            End If
                                        Case Txt29Assenti.ID
                                            If (Txt29Assenti.Text = String.Empty) Then
    ...
    Private Sub ControllaNoteAbilitate()
        'Antonello
        If Txt24Assenti.Text <> "" Then
            Txt24Assenti.Enabled = True
        Else
            Txt24Assenti.Enabled = False
        End If
        If Txt24Presenti.Text <> "" Then
            Txt24Presenti.Enabled = True
        Else
            Txt24Presenti.Enabled = False
        End If
        If Txt24EsteroAssenti.Text <> "" Then
            Txt24EsteroAssenti.Enabled = True
        Else
            Txt24EsteroAssenti.Enabled = False
        End If
        If Txt25Assenti.Text <> "" Then
            Txt25Assenti.Enabled = True
        Else
            Txt25Assenti.Enabled = False
        End If
        If Txt28Assenti.Text <> "" Then
            Txt28Assenti.Enabled = True
        Else
            Txt28Assenti.Enabled = False
        End If
        If Txt29Assenti.Text <> "" Then
            Txt29Assenti.Enabled = True
        Else
            Txt29Assenti.Enabled = False
        End If

        strsql = "select VociParametri.idvoce,VociParametri.AssociaNote,Vociparametri.CampoNote, " & _
        " storicoProgetti.noteStorico,storicoProgetti.Note24A, storicoProgetti.Note24B, storicoProgetti.Note24C, storicoProgetti.Note25, storicoProgetti.Note28, storicoProgetti.Note29" & _
        " from VociParametri " & _
        " inner join storicovoci on (storicovoci.idvoce=VociParametri.idvoce)" & _
        " inner join storicoProgetti on (storicovoci.idStorico=StoricoProgetti.idStorico)" & _
        " where idprogetto=" & hf_IdProgetto.Value & "  and VociParametri.AssociaNote=1 and storicovoci.idStorico= "
        If hf_Personalizza.Value = "Modifica" Then
            strsql = strsql & "" & hf_IdStorico.Value & ""
        Else
            strsql = strsql & " (select max(idstorico)as maxid from storicoProgetti where idprogetto=" & hf_IdProgetto.Value & ")"
        ...
        Do While dtrgenerico.Read
            If lblNazione.Text = "Italiana" Then
                Select Case dtrgenerico("CampoNote")
                    Case Txt24Assenti.ID
                        Txt24Assenti.Enabled = True
                    Case Txt24Presenti.ID
                        Txt24Presenti.Enabled = True
    ...
    Private Function InsertTMP(ByVal UserName As String, ByVal ContaCeck As Integer) As Integer
        ...
        strsql = "Insert into storicoProgettiTMP (idprogetto,dataInserimento," & _
                ...
                "(" & hf_IdProgetto.Value & ",getdate()," & txtPuntoRilevanza.Text & "," & txtPuntoCoerenza.Text & "," & IIf(ddlinternet.SelectedValue.ToString = "", "null", ddlinternet.SelectedValue) & "," & _
                "" & txtDefCopertura.Text & "," & txtdefInflazione.Text & "," & hf_StatoProgetto.Value & "," & _
                "'" & UserName & "', '" & Replace(txtnote.Text, "'", "''") & "'," & txtPuntRegio.Text & ", "
        ...
        For k = 0 To dtDomanda.Rows.Count - 1
            ...
            For w = 0 To dtRisposta.Rows.Count - 1
                ...
                For s = 1 To j
                    ...
                    InserisciStoricoVociTMP("s", panelCaratteristicheProgetto, nomech, idStoricoTMP)
    ...
    Private Function StoreVerificaValutazioneQual(ByVal IdStoricoTMP As Integer, ByRef StrMessaggio As String) As Integer
        ...
        CustOrderHist.CommandText = "SP_VERIFICA_VALUTAZIONE_QUAL"
    ...
    Private Sub imgRiepilogoDocumentiVol_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgAssociaDocumentiProg.Click
        Response.Write("<script>")
        Response.Write("window.open('WfrmAssociaDocumentiProgetti.aspx?IdAttivita=" & hf_IdProgetto.Value & "', 'AssociaDocumenti', 'width=900, height=600,dependent=no,scrollbars=yes,status=si')")
        Response.Write("</script>")
    End Sub
    ...
    Protected Sub imCalcDefl_Click(ByVal sender As Object, ByVal e As EventArgs) Handles imCalcDefl.Click
        Response.Write("<script>")
        Response.Write("window.open('WfrmCalcDeflettori.aspx?VengoDa=" & Costanti.VENGO_DA_VALUTAZIONE_QUALITA & "','Calcolatore','height=600,width=500,dependent=no,scrollbars=yes,status=no,resizable=yes');")
        Response.Write("</script>")
    End Sub
    ...
    End Sub
    Protected Sub CalcPerc_Click(ByVal sender As Object, ByVal e As EventArgs) Handles CalcPerc.Click
        Response.Write("<SCRIPT>" & vbCrLf)
        Response.Write("window.open('WfrmRisFinanzAgg.aspx?popup=2&IdAttivita=" & hf_IdProgetto.Value & "','Calcolatore','height=390,width=600,dependent=no,scrollbars=yes,status=no,resizable=yes');" & vbCrLf)
        Response.Write("</SCRIPT>")
    End Sub
    ...
    Private Function ValidaDati() As Boolean
        ...
        If (Integer.TryParse(txtPuntoRilevanza.Text, numeroIntero) = False) Then
            errore.AppendLine("Il valore inserito per Punteggio Rilevanza non è valido. <br/>")
        ElseIf Not (valoriConsentiti.Contains(numeroIntero)) Then
            errore.AppendLine("Il Punteggio Rilevanza accetta solo i seguenti valori: ")
            errore.AppendLine(valoriConsentiti.ToString())
            errore.AppendLine(". <br/>")
            numeroIntero = 0
        End If


        If (Integer.TryParse(txtPuntoCoerenza.Text, numeroIntero) = False) Then
            errore.AppendLine("Il valore inserito per Punteggio Coerenza non è valido. <br/>")
        ElseIf Not (valoriConsentiti.Contains(numeroIntero)) Then
            errore.AppendLine("Il Punteggio Coerenza accetta solo i seguenti valori: ")
            errore.AppendLine(valoriConsentiti.ToString())
            errore.AppendLine(". <br/>")
            numeroIntero = 0
        End If
        ...
        If (Integer.TryParse(txtDefCopertura.Text, numeroIntero) = False) Then
            errore.AppendLine("Il valore inserito per Deflettore Copertura non è valido. <br/>")
        ElseIf (numeroIntero > valoreMassimoDeflettoreCopertura) Then
            errore.AppendLine("Il valore inserito per Deflettore Copertura non può essere superiore a ")
            errore.AppendLine(valoreMassimoDeflettoreCopertura.ToString())
            errore.AppendLine(". <br/>")
            numeroIntero = 0
        End If


        If (Integer.TryParse(txtdefInflazione.Text, numeroIntero) = False) Then
            errore.AppendLine("Il valore inserito per Deflettore Inflazione non è valido. <br/>")
        ElseIf (numeroIntero > valoreMassimoDeflettoreInflazione) Then
            errore.AppendLine("Il valore inserito per Deflettore Inflazione non può essere superiore a ")
            errore.AppendLine(valoreMassimoDeflettoreInflazione.ToString())
            errore.AppendLine(". <br/>")
            numeroIntero = 0
        End If

        If CInt(txtDefSanzione.Text) >= 0 Then
            If (Integer.TryParse(txtDefSanzione.Text, numeroIntero) = False) Then
                errore.AppendLine("Il valore inserito per Deflettore Sanzione non è valido. <br/>")
            ElseIf (numeroIntero > valoreMassimoDeflettoreSanzione) Then
                errore.AppendLine("Il valore inserito per Deflettore Sanzione non può essere superiore a ")
                errore.AppendLine(valoreMassimoDeflettoreSanzione.ToString())
                errore.AppendLine(". <br/>")
                numeroIntero = 0
            End If
        Else

            errore.AppendLine("Il valore inserito Negativo per Deflettore Sanzione non è valido. <br/>")
        End If

        If CInt(txtDefInfortuni.Text) >= 0 Then
            If (Integer.TryParse(txtDefInfortuni.Text, numeroIntero) = False) Then
                errore.AppendLine("Il valore inserito per Deflettore Infortuni non è valido. <br/>")
            ElseIf (numeroIntero > valoreMassimoDeflettoreInfortuni) Then
                errore.AppendLine("Il valore inserito per Deflettore Infortuni non può essere superiore a ")
                errore.AppendLine(valoreMassimoDeflettoreInfortuni.ToString())
                errore.AppendLine(". <br/>")
                numeroIntero = 0
            End If
        Else
            errore.AppendLine("Il valore inserito Negativo per Deflettore Infortuni non è valido. <br/>")
        End If


        If (tipoProgetto = 0) Then
            If (ddlinternet.SelectedValue = String.Empty) Then
                errore.AppendLine("Selezionare un valore per Punteggio Intenet. <br/>")
            End If

        End If

        ' Dim PunteggioRegio As Double = txtPuntRegio.Text
        If CDbl(txtPuntRegio.Text) >= 0 Then

            txtPuntRegio.Text = ClsServer.RendiFormatPunto(txtPuntRegio.Text)

            If (tipoProgetto = 1) Then
                If (Integer.TryParse(txtPuntRegio.Text, numeroIntero) = False) Then
                    errore.AppendLine("Il valore inserito per Punteggio Regionale non è valido. <br/>")
                ElseIf (numeroIntero > valoreMassimoPunteggioRegionale) Then
                    errore.AppendLine("Il valore inserito per Punteggio Regionale non può essere superiore a ")
                    errore.AppendLine(valoreMassimoPunteggioRegionale.ToString())
                    errore.AppendLine(". <br/>")
                    numeroIntero = 0
                End If
            End If
        Else
            errore.AppendLine("Il valore inserito Negativo per Punteggio Regionale non è valido. <br/>")
        End If
    ...
    End Sub
    Protected Sub imgCalcolaCoerenza_Click(ByVal sender As Object, ByVal e As EventArgs) Handles imgCalcolaCoerenza.Click
        Response.Write("<SCRIPT>" & vbCrLf)
        Response.Write("window.open('WfrmCalcoloRilevanzaCoerenza.aspx?Descrizione=" & txtTOTALEPUNTEGGIODESCRIZIONE.Value & "&Contesto=" & txtTOTALEPUNTEGGIOCONTESTO.Value & "&Obiettivi=" & txtTOTALEPUNTEGGIOOBIETTIVI.Value & "&TipoProgetto=" & txtNazionale.Value & "','Calcola','height=500,width=500,dependent=no,scrollbars=yes,status=no,resizable=yes');" & vbCrLf)
        Response.Write("</SCRIPT>")
    End Sub
    ...
    Private Sub MascheraINLettura()
        ...
        CalcPerc.Visible = False
        imgCalcolaCoerenza.Visible = False
        imgElencoDocumentiProg.Visible = False
        imgAssociaDocumentiProg.Visible = False
        imCalcDefl.Visible = False
    ...
    Private Function VerificaGmoTutoraggioMinoriOpp(ByVal idProgetto As Integer)
        ...
        If dtrgenerico("GiovaniMinoriOpportunità") = True Then
            panelMinoriOpportunita.Visible = True
            panelMinoriOpportunita.Enabled = True
            LinkButton4.Visible = True
        Else
            panelMinoriOpportunita.Enabled = False
            panelMinoriOpportunita.Visible = False
