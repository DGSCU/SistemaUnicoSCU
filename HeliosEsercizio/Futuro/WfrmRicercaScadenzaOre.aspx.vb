...
Imports System.IO
Public Class WfrmRicercaScadenzaOre
    ...
    Private Sub EseguiRicerca(ByVal bytVerifica As Byte, Optional ByVal bytpage As Integer = 0)
        ...
        'AUTORE: Guido Testa 
        'DATA: 18/12/2006
        'DESCRIZONE: ricerco tutti i progetti con scadenza caricamento ore entro i 150gg dalla data inizio progetto
        ...
        strquery = "Select codiceregione, codiceprogetto, titolo, datainizioattività, giornirestanti, datascadenza,ente,email,telefono,fax,Case isnull(SegnalazioneSanzione,0) When 0 then 'No' When 1 then '<img src=images/Anomalie.bmp onclick=VisualizzaSanzioneProg('+ convert(varchar, IDAttività) + ','+ convert(varchar, IDEntePresentante) + ') STYLE=cursor:hand title=Sanzione border=0>' End as SegnalazioneSanzione From VW_SCADENZE_IMPORT_ORE_2 "

        'imposto eventuali parametri  
        If txtTitoloProgetto.Text <> "" Then
            strquery = strquery & strCond & " titolo like '" & ClsServer.NoApice(Trim(txtTitoloProgetto.Text)) & "%'"
            strCond = " And "
        End If

        If txtCodProg.Text <> "" Then
            strquery = strquery & strCond & " codiceprogetto like '" & ClsServer.NoApice(Trim(txtCodProg.Text)) & "%'"
            strCond = " And "
        End If
        ...
        If ddlSegnalazioneSanzione.SelectedItem.Text <> "Tutti" Then
            strquery = strquery & strCond & " isnull(SegnalazioneSanzione,0) =" & ddlSegnalazioneSanzione.SelectedValue
            strCond = " And "
        End If
        'FiltroVisibilita 02/12/20104 da s.c.
        ...
        dgRisultatoRicerca.Columns(6).Visible = ClsUtility.ForzaPresenzaSanzioneProgetto(Session("Utente"), Session("conn"))
        ...
        If dgRisultatoRicerca.Items.Count = 0 Then 'se la griglia e vuota la nascondo
            ...
            imgStampa1.Visible = False
        Else
            imgStampa1.Visible = True
    ...
    Sub CarciaDatiStampa(ByVal dtsGenerico As DataSet)
        ...
        NomeColonne(1) = "Cod.Progetto"
        NomeColonne(2) = "Titolo Progetto"
    ...
    Protected Sub imgStampa1_Click(sender As Object, e As EventArgs) Handles imgStampa1.Click
        imgStampa1.Visible = False
    ...
    Function StampaCSV(ByVal DTBRicerca As DataTable)
        ...
        Else
            ...
            NomeUnivoco = xPrefissoNome & "ExpElencoScadenzaOre" & Year(Now) & Month(Now) & Day(Now) & Hour(Now) & Minute(Now) & Second(Now)
