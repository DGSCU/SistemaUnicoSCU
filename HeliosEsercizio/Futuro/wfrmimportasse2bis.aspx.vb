...
Public Class wfrmimportasse2bis
    ...
    Private Sub CmdElabora_Click(ByVal sender As Object, ByVal e As EventArgs) Handles CmdElabora.Click
        ...
        If txtSelFile.PostedFile.FileName.ToString <> "" Then
            Try
                NomeUnivoco = "VolAsse1Bis" & Session("IdEnte") & "_" & Session("Utente") & "_" & Year(Now) & Month(Now) & Day(Now) & Hour(Now) & Minute(Now) & Second(Now)
                Dim file As String
    ...
    Private Sub CreaTabTemp()
        ...
        '--- CREO TAB TEMPORANEA
        strSql = "CREATE TABLE [#TEMP_VolAsse2Bis] (" & _
                 ...
                 "[CodiceVolontario] [nvarchar] (1000) COLLATE DATABASE_DEFAULT, " & _
                 "[Cognome] [nvarchar] (1000) COLLATE DATABASE_DEFAULT, " & _
                 "[Nome] [varchar] (1000) COLLATE DATABASE_DEFAULT, " & _
                 "[CodiceFiscale] [varchar] (1000) COLLATE DATABASE_DEFAULT, " & _
                 "[PresenzaInItalia] [varchar] (1000) COLLATE DATABASE_DEFAULT, " & _
                 "[TitoloStudio] [varchar] (1000) COLLATE DATABASE_DEFAULT, " & _
                 "[CondizioneOccupazionale] [varchar] (1000) COLLATE DATABASE_DEFAULT, " & _
    ...
    Private Sub LeggiCSV()
        ...
        While (xLinea <> "")
            ...
            If swErr = False Then
                'effetto ulteriori controlli Asse2Bis
                ControlliImport(ArrCampi(3), ArrCampi(6), ArrCampi(7), ArrCampi(8), ArrCampi(9), intEsito, strMessaggio)
        ...
        Reader.Close()
        Writer.Close()
        Response.Redirect("WfrmRisultatoImportasse2bis.aspx?NomeFile=" & NomeUnivoco & "&Tot=" & Tot & "&TotOk=" & TotOk & "&TotKo=" & TotKo)
    ...
    Private Sub ScriviTabTemp(ByVal pArray() As String)
        ...
        Try
            strsql = "INSERT INTO #TEMP_VolAsse2Bis " & _
                     "(CodiceProgetto, " & _
                     "TitoloProgetto, " & _
    ...
    Protected Sub imgEsporta_Click(sender As Object, e As EventArgs) Handles imgEsporta.Click
        ...
        EsportazioneVolAsse2Bis()
    End Sub

    Sub EsportazioneVolAsse2Bis()
        ...
        myDataTable = ExecuteDataTable("SP_ASSE2BIS_EXPORT_VOLONTARI", arrParam)
        myDataSet.Tables.Add(myDataTable)
        OutputXls("ExpVolAsse1Bis", nomeFile)
        ...
        Else
            lblMessaggioErrore.Visible = True
            lblMessaggioErrore.Text = " Non risultano volontari per i quali è necessario inserire i dati."
    ...
    'NOTE
    Public Sub PresenzaInItalia()


        Dim strSQL As String
        Dim dsPresenzaInItalia As SqlDataReader

        strSQL = "Select Descrizione, Codice From Asse2Bis_PresenzaInItalia  "
        dsPresenzaInItalia = ClsServer.CreaDatareader(strSQL, Session("conn"))
        Response.Write("<fieldset>")
        Response.Write("<ul>")
        Do While dsPresenzaInItalia.Read
            Response.Write("<li><strong>" & dsPresenzaInItalia.Item("Codice") & "</strong> se <strong>" & dsPresenzaInItalia.Item("Descrizione") & "</strong></li>")
        Loop
        Response.Write("</ul>")
        Response.Write("</fieldset>")
        dsPresenzaInItalia.Close()
        dsPresenzaInItalia = Nothing
    End Sub


    Public Sub TitoloStudio()

        Dim strSQL As String
        Dim dsTitoloStudio As SqlDataReader

        strSQL = "Select Descrizione, Codice From Asse2Bis_TitoloStudio"
        dsTitoloStudio = ClsServer.CreaDatareader(strSQL, Session("conn"))
        Response.Write("<fieldset>")
        Response.Write("<ul>")
        Do While dsTitoloStudio.Read
            Response.Write("<li><strong>" & dsTitoloStudio.Item("Codice") & "</strong> se <strong>" & dsTitoloStudio.Item("Descrizione") & "</strong></li>")
        Loop
        Response.Write("</ul>")
        Response.Write("</fieldset>")
        dsTitoloStudio.Close()
        dsTitoloStudio = Nothing
    End Sub

    Public Sub CondizioneOccupazionale()

        Dim strSQL As String
        Dim dsCondizioneOccupazionale As SqlDataReader

        strSQL = "Select Descrizione, Codice From Asse2Bis_CondizioneOccupazionale "
        dsCondizioneOccupazionale = ClsServer.CreaDatareader(strSQL, Session("conn"))
        Response.Write("<fieldset>")
        Response.Write("<ul>")
        Do While dsCondizioneOccupazionale.Read
            Response.Write("<li><strong>" & dsCondizioneOccupazionale.Item("Codice") & "</strong> se <strong>" & dsCondizioneOccupazionale.Item("Descrizione") & "</strong></li>")
        Loop
        Response.Write("</ul>")
        Response.Write("</fieldset>")
        dsCondizioneOccupazionale.Close()
        dsCondizioneOccupazionale = Nothing
    End Sub
    ...
    Private Sub ControlliImport(ByVal strCodiceVolontario As String, ByVal strCodiceFiscale As String, ByVal strPresenzaInItalia As String, ByVal strTitoloStudio As String, strCondizioneOccupazionale As String, ByRef intEsito As Integer, ByRef strMessaggio As String)
        ...
        Try
            SqlCmd.CommandText = "SP_ASSE2BIS_CONTROLLI_IMPORT"
            ...
            SqlCmd.Parameters.Add("@CodiceVolontario", SqlDbType.VarChar, 1000).Value = strCodiceVolontario
            SqlCmd.Parameters.Add("@CodiceFiscale", SqlDbType.VarChar, 1000).Value = strCodiceFiscale
            SqlCmd.Parameters.Add("@PresenzaInItalia", SqlDbType.VarChar, 1000).Value = strPresenzaInItalia
            SqlCmd.Parameters.Add("@TitoloStudio", SqlDbType.VarChar, 1000).Value = strTitoloStudio
            SqlCmd.Parameters.Add("@CondizioneOccupazionale", SqlDbType.VarChar, 1000).Value = strCondizioneOccupazionale
