...
Public Class WfrmIscrizione
	...
	Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Page.IsPostBack Then
            ...
            Finally
                ...
                End If
            End Try
            If IdClasse = CLASSE_NAZIONALE Then
                txtSezione.Text = "Nazionale"
            ElseIf IdClasse = CLASSE_REGIONALE Then
                txtSezione.Text = "della " & Globalization.CultureInfo.CurrentCulture.TextInfo.ToTitleCase(Sezione.ToLower())
            End If
            hfCodiceFiscaleEnte.Value = CodiceFiscale
            txtEnte.Text = NomeEnte
            txtNumeroSedi.Text = NumeroSedi
	...
	Private Function CaricaRiepilogo() As Ente
        ...
        'Caricamento RuoliAntimafia
        '--- recupero dati EnteFaseAntimafia  NON CANCELLARE LE QUATTRO RIGHE SEGUENTI la Session("IdEnteFaseAntimafia") viene usata nella btnPresenta_Click
        Session("IdEnteFaseAntimafia") = Nothing
        Dim _info As New clsRuoloAntimafia.InfoAdeguamentoAntimafia(idEnte, Session("conn"), False)
		'If _info.IdEnteFaseAntimafia = 0 Or _info.isAperto = 0 Or _info.isAccreditamento = 0 Then Throw New Exception("Errore Caricamento dati RuoliAntimafia")
		If _info.IdEnteFaseAntimafia = 0 Then Throw New Exception("Errore Caricamento dati RuoliAntimafia")
		Session("IdEnteFaseAntimafia") = _info.IdEnteFaseAntimafia

		Dim RuoliAntimafia As New List(Of RuoloAntimafia)
        Dim sqlRuoliAntimafia As String = "SELECT IdEnte, R.RuoloAntiMAfia Ruolo,  Nome + ' ' + Cognome Nome, CodiceFiscale, Email, Telefono, DataNascita, CASE WHEN PN.IDProvincia IS NULL THEN CN.Denominazione ELSE CN.Denominazione + ' (' + PN.DescrAbb + ')' END LuogoNascita FROM RuoliAntimafia E  LEFT JOIN	comuni CR on CR.IDComune = E.IDComuneResidenza  LEFT JOIN	provincie PR on PR.IDProvincia = CR.IDProvincia LEFT JOIN	comuni CN on CN.IDComune = E.IDComuneNascita LEFT JOIN	provincie PN on PN.IDProvincia = CN.IDProvincia JOIN ElencoRuoliAntimafia R ON R.Id = E.IdElencoRuoliAntimafia WHERE IdEnteFaseAntimafia=@IdEnteFaseAntimafia and IDEnte IN (select IDEnteFiglio from entirelazioni  where IdEntePadre=@IdEnte UNION Select @IdEnte)"
        Dim cmdRuoliAntimafia As New SqlCommand(sqlRuoliAntimafia, Session("conn"))
        cmdRuoliAntimafia.Parameters.AddWithValue("@IdEnte", idEnte)
        cmdRuoliAntimafia.Parameters.AddWithValue("@IdEnteFaseAntimafia", _info.IdEnteFaseAntimafia)
        ...
        Try
            ...
            While rdrRuoliAntimafia.Read()
                ...
                RuoloAntimafia.IdEnte = GetData(rdrRuoliAntimafia("IdEnte"))
                RuoloAntimafia.Ruolo = GetData(rdrRuoliAntimafia("Ruolo"))
                RuoloAntimafia.Nome = GetData(rdrRuoliAntimafia("Nome"))
                RuoloAntimafia.CodiceFiscale = GetData(rdrRuoliAntimafia("CodiceFiscale"))
                RuoloAntimafia.Email = GetData(rdrRuoliAntimafia("Email"))
                RuoloAntimafia.Telefono = GetData(rdrRuoliAntimafia("Telefono"))
                RuoloAntimafia.DataNascita = GetData(rdrRuoliAntimafia("DataNascita"))
                RuoloAntimafia.LuogoNascita = GetData(rdrRuoliAntimafia("LuogoNascita"))
        ...
        Finally
            ...
            End If
        End Try
        For Each ruoloAntimafia In RuoliAntimafia
            If Enti.ContainsKey(ruoloAntimafia.IdEnte) Then
                Enti(ruoloAntimafia.IdEnte).RuoliAntimafia.Add(ruoloAntimafia)
            Else
                Throw New Exception(Format("Errore inserimento Ruolo Antimafia: IdEnte {0} Non presente ", ruoloAntimafia.IdEnte))
            End If
        Next
        Return ente
    End Function
	Private Sub Riepilogo_Click() Handles Riepilogo.Click
		...
		Try
			documento.open(Server.MapPath("download\Master\Iscrizione\Riepilogo.docx"))
		Catch ex As Exception
		...
		Response.AddHeader("Content-Disposition", "attachment; filename=" & "DomandaIscrizione.pdf")
	...
	Private Function Controlli() As String
		...
		If Not chkRichiestaIscrizione.Checked Then
			errori = errori + "Non è stata spuntata la richiesta di iscrizione.</br>"
		End If
	...
	Protected Sub btnPresenta_Click(sender As Object, e As EventArgs)
		...
		If documentoRiepilogo Is Nothing Then
			errori = errori + "Non è stato caricata la Domanda di Iscrizione.</br>"
		End If
		...
		If Not ConfigurationSettings.AppSettings("DisabilitaFirma") = "true" Then
			...
			If Not sc.checkSignature(rappresentanteLegale.CodiceFiscale) Then
				Log.Warning(LogEvent.FIRMA_NON_VALIDA, sc.getLog())
				lblError.Text = ("Il documento non è firmato digitalmente o non è firmato dal Rappresentante Legale")
				Exit Sub
            End If
            ...
            If IsNothing(_documentoDaFirmare) Then
                Log.Warning(LogEvent.FIRMA_NON_VALIDA, message:="Non è stata scaricata la Domanda di Iscrizione da firmare ed allegare")
                lblError.Text = ("Non è stata scaricata la Domanda di Iscrizione da firmare ed allegare")
                Exit Sub
            End If

            If Not sc.compareSignedNotSigned(documentoRiepilogo.Blob, _documentoDaFirmare) Then
                Log.Warning(LogEvent.FIRMA_NON_VALIDA, message:="Il file firmato non corrisponde alla Domanda di Iscrizione da firmare ed allegare")
                lblError.Text = ("Il file firmato non corrisponde alla Domanda di Iscrizione da firmare ed allegare")
                Exit Sub
            End If
		...
		Try
            ...
            Dim idEnteFase As Integer = RicavaIdEnteFase(Session("IdEnte")) 'variabile potenzialmente duplicata, mantenuta ma da sostituire con strIdEntefase
			...
			If myDataReader.Item(0) = True Then
				...
				Dim idRiepilogo As Integer = SalvaAllegato(documentoRiepilogo, TipoFile.RICHIESTA_ISCRIZIONE, strIdEntefase, insertRiepilogoCommand)
				...
				myQuerySql = "insert into ProtocolloDomanda (Idente,TipoDomanda,IdAllegatoDomanda) VALUES (" & Session("IdEnte") & ",1," & idRiepilogo & ")"
				CmdAggiornaDB = New SqlClient.SqlCommand(myQuerySql, IIf(ClsServer.Connessione(Session("TipoUtente"), Session("IdStatoEnte"), Session("ConnSnapshot"), Session("conn"), Session("FlagForzatura")) = False, Session("ConnSnapshot"), Session("Conn")), MyTransaction)
				CmdAggiornaDB.ExecuteNonQuery()
			Else
				myDataReader.Close()
		...
		'If _info.IdEnteFaseAntimafia = 0 Or _info.isAperto = 0 Or _info.isAccreditamento = 0 Then Throw New Exception("Errore Caricamento dati RuoliAntimafia")
		If _info.IdEnteFaseAntimafia = 0 Or _info.isAperto = 0 Then Throw New Exception("Errore Caricamento dati RuoliAntimafia")

		Session("IdEnteFaseAntimafia") = _info.IdEnteFaseAntimafia

        myQuerySql = "update EntiFasiAntimafia set Stato=3,DataChiusuraFase=GetDate() WHERE IDEnte=" & Session("IdEnte") & " AND IdEnteFaseAntimafia=" & Session("IdEnteFaseAntimafia") & " AND IdEnteFaseRiferimento=" & intIDEnteFase
        CmdAggiornaDB = New SqlClient.SqlCommand(myQuerySql, Session("conn"), MyTransaction)
        CmdAggiornaDB.ExecuteNonQuery()
    ...
    Protected Sub btnUploadRiepilogo_Click(sender As Object, e As EventArgs)
        ...
        Dim allegato As New Allegato() With {
         .Blob = bBLOBStorage,
         .Filename = "DOMANDAISCRIZIONE_" & hfCodiceFiscaleEnte.Value & estensione,
         .Hash = hashValue,
	...
	Protected Sub btnDownloadRiepilogo_Click(sender As Object, e As ImageClickEventArgs)
		...
		Log.Information(LogEvent.PRESENTAZIONE_DOWNLOAD, "Domanda di Iscrizione caricata", entity:=entity)
		Response.Clear()
		Response.ContentType = "Application/pdf"
	...
	Function CodiceGenerato(ByVal idclasseselezionata As Integer, Optional ByVal Transazione As System.Data.SqlClient.SqlTransaction = Nothing) As String
        ...
        strLocal = "select top 1 * from "
        strLocal += "("
        strLocal += "SELECT ISNULL(replicate('0', len(max(CodiceRegione)) - 2 - len((max(right(CodiceRegione,len(CodiceRegione) - 2)) + 1))) + convert(varchar,max(right(CodiceRegione,len(CodiceRegione) - 2)) + 1),'00001') as MAXX from enti where len(codiceregione)=7 AND LEFT(CodiceRegione,2)='CP'"
        strLocal += " union "
        strLocal += "SELECT ISNULL(replicate('0', len(max(CodiceProvvisorio)) - 2 - len((max(right(CodiceProvvisorio,len(CodiceProvvisorio) - 2)) + 1))) + convert(varchar,max(right(CodiceProvvisorio,len(CodiceProvvisorio) - 2)) + 1),'00001') as MAXX from enti where len(CodiceProvvisorio)=7 AND LEFT(CodiceProvvisorio,2)='CP'"
        strLocal += ") t"
        strLocal += " order by 1 desc"
