...
Public Class WfrmRicAccettazioneIstanzaUNSCProgrammi
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If IsPostBack = False Then
            ...
            If Request.QueryString("Vengoda") = "Accettazione" Then
                ...
                txtDenominazioneEnte.Text = Request.QueryString("Ente")
                DdlBando.SelectedIndex = Request.QueryString("Bando")
                ddlanno.SelectedIndex = Request.QueryString("Anno")
                ddlStatoAttivita.SelectedIndex = Request.QueryString("StatoAttivita")
                CboCompetenza.SelectedIndex = Request.QueryString("Competenza")

                If Not IsNothing(Session("LocalDataSet")) Then
    ...
    Sub caricacomboanno()
        ...
        " union" & _
        " select distinct left(anno,4) as Anno from bando where bando.programmi = 1 order by 1", Session("conn"))
    ...
    Private Sub CaricaPrima()
        ...
        ddlStatoAttivita.DataSource = MakeParentTable("select IdStatoBandoProgramma, StatoBandoProgramma from statiBandiProgrammi")
        ...
        If Not dtrgenerico Is Nothing Then
            ...
            dtrgenerico = Nothing
        End If
        dtrgenerico = ClsServer.CreaDatareader("select IdStatoBandoProgramma from statiBandiProgrammi where DaValutare=1", Session("conn"))
    ...
    Sub ricerca()
        ...
        Dim strquery As String = "select a.idbando,e.idente,e.denominazione, e.CodiceRegione, a.bando," & _
            " convert(varchar,a.datainiziovalidità,103) as datainizio," & _
            " convert(varchar,a.datafinevalidità,103) as datafine," & _
            " count(c.idattività) as progetti, " & _
            " count(distinct c.idprogramma) as programmi, " & _
            " d1.statobandoprogramma as Stato,b1.idbandoprogramma,RegioniCompetenze.Descrizione as Competenza from bando a" & _
            " inner join bandiattività b on a.idbando=b.idbando" & _
            " inner join bandiprogrammi b1 on b.idente = b1.idente and b.idbando=b1.idbando" & _
            " inner join attività c on b.idbandoattività=c.idbandoattività" & _
        ...
        strquery = strquery & " LEFT JOIN BANDORICORSI ON c.IDBANDORICORSO = BANDORICORSI.IDBANDORICORSO " & _
            " inner join StatiBandiAttività d on d.idstatobandoattività=b.idstatobandoattività" & _
            " inner join StatiBandiProgrammi d1 on d1.idstatobandoprogramma=b1.idstatobandoprogramma" & _
            " inner join enti e on b.idente=e.idente" & _
            ...
            " where 1=1 and a.programmi=1 and AssociaUtenteGruppo.username='" & Replace(Session("Utente"), "'", "''") & "' "
        ...
        If ddlanno.SelectedItem.Text <> "Seleziona" Then
            strquery = strquery & " and left(a.anno,4)=" & CInt(ddlanno.SelectedItem.Text) & ""
        End If
        ...
        If ddlStatoAttivita.SelectedValue <> 0 Then
            strquery = strquery & " and d1.idstatobandoprogramma=" & ddlStatoAttivita.SelectedValue & ""
        End If
        ' Filtro per regioni
        ...
        strquery = strquery & " group by b.idente,a.idbando, a.bando," & _
            " a.datainiziovalidità, a.datafinevalidità,d1.statobandoprogramma,e.denominazione,e.CodiceRegione,e.idente,b1.idbandoprogramma,RegioniCompetenze.Descrizione order by a.datainiziovalidità asc"
    ...
    Sub CaricaDataTablePerStampa(ByVal DataSetDaScorrere As DataSet)
        ...
        NomiCampiColonne(6) = "programmi"
        NomiCampiColonne(7) = "Competenza"
    ...
    Private Sub dgRisultatoRicerca_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles dgRisultatoRicerca.ItemCommand
        Select Case e.CommandName
            Case "Select"
                ...
                'Response.Redirect("WfrmProgrammiIstanza.aspx?idBP=" & e.Item.Cells(10).Text & "") '&DataFine=" & e.Item.Cells(7).Text & "&DataInizio=" & e.Item.Cells(6).Text & "&Stato=" & e.Item.Cells(12).Text & "&Arrivo=WfrmRicAccettazioneIstanzaUNSC.aspx&idBA=" & e.Item.Cells(10).Text & "")
                Response.Redirect("WfrmProgrammiIstanza.aspx?idBP=" & e.Item.Cells(10).Text & "&Ente=" & txtDenominazioneEnte.Text & "&Bando=" & DdlBando.SelectedIndex & "&Anno=" & ddlanno.SelectedIndex & "&StatoAttivita=" & ddlStatoAttivita.SelectedIndex & "&Competenza=" & CboCompetenza.SelectedIndex & "&Vengoda=Accettazione" & "")
