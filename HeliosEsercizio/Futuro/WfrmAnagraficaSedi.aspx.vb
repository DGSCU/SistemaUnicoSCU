...
Public Class WfrmAnagraficaSedi
    Inherits SmartPage
        ...
        If IsPostBack = False Then
            ...
            If Session("TipoUtente") = "E" Then
                ...
                If ClsUtility.ControllaStatoEntePerBloccareMaschereAnagrafica(Session("IdEnte"), IIf(ClsServer.Connessione(Session("TipoUtente"), Session("IdStatoEnte"), Session("ConnSnapshot"), Session("conn"), Session("FlagForzatura")) = False, Session("ConnSnapshot"), Session("Conn"))) = True Then
                    ...
                    '    divFuoriAdeguamento.Visible = True
                    'End If
                    GestisciEstero()
                    Exit Sub
            ...
            If lblPersonalizza.Value <> "Inserimento" Then
                ...
                Else
                    ...
                    Else
                        ...
                        If UCase(lblStato.Text) = "CANCELLATA" Then
...
#Region "Operazioni DB"
    ...
    Private Sub Inserimento()
        ...
        If ControlloDoppioneSedeNuova(IIf(ddlEntiFigli.SelectedValue = 0, Session("IdEnte"), ddlEntiFigli.SelectedValue), txtIndirizzo.Text, txtCivico.Text, ddlComune.SelectedValue, Trim(TxtPalazzina.Text), Trim(TxtScala.Text), Trim(TxtPiano.Text), Trim(TxtInterno.Text), "I") = True Then
            cmdSalva.Visible = True
            'Inserisco il Messaggio di Errore
            msgErrore.Text = "Sede già presente in archivio con uguale indirizzo."
            Log.Warning(LogEvent.SEDI_INSERIMENTO_ERRORE, "Sede già presente in archivio con uguale indirizzo.")
            Exit Sub
        End If
        ...
        End If
            ...
            ' Salva Allegato 
            'Creazione Allegato
            'CaricoComboTitoloGiuridico la fase dell'ente
            Dim managerEnte = New clsEnte()


            Dim strIdEntefase As String = managerEnte.RicavaIdEnteFase(Session("IdEnte"), Session("conn"))

            'Dim sqlFase As String = "Select EntiFasi.IdEnteFase, EntiFasi.DataInizioFase, EntiFasi.DataFineFase" &
            '                        " From EntiFasi  left Join EntiFasi e on EntiFasi.IdEnteFaseRiferimento = e.IdEnteFase" &
            '                        " Where EntiFasi.IdEnte = " & Session("idEnte") & " And EntiFasi.stato = 1 And EntiFasi.TipoFase in (1,2,3)" &
            '                        " And GETDATE() between EntiFasi.DataInizioFase And EntiFasi.DataFineFase" &
            '                        " Order by EntiFasi.DataFineFase Desc"

            'ChiudiDataReader(dtrGenerico)

            'dtrGenerico = ClsServer.CreaDatareader(sqlFase, IIf(ClsServer.Connessione(Session("TipoUtente"), Session("IdStatoEnte"), Session("ConnSnapshot"), Session("conn"), Session("FlagForzatura")) = False, Session("ConnSnapshot"), Session("Conn")))
            'If dtrGenerico.HasRows = True Then
            '    dtrGenerico.Read()
            '    strIdEntefase = dtrGenerico("IdEnteFase")
            'End If
            'ChiudiDataReader(dtrGenerico)
            ...
            Dim idAllegato As Integer = SalvaAllegato(LSE, TipoFile.LETTERA_SEDE_ESTERA, strIdEntefase)
        ...
        If ControllaEnteFiglio(ddlEntiFigli.SelectedValue) = True Then
            ...
            If ddlTipologia.SelectedItem.Text = "Operativa" Then
                ...
                strsql = "insert into entiSediattuazioni (Denominazione,identeSede,UsernameStato,idStatoEnteSede,UsernameInseritore,DataInserimento,IdEnteCapofila,Certificazione,DataCertificazione ,UserCertificazione ,NMaxVolontari,FlagMaxVolontari, UsernameMaxVolontari,Note ) " & _
                ...
                strsql = strsql & " from StatiEntiSedi " & _
    ...
    Private Sub Modifica()
        ...
        'agg. da simona cordella il 16/06/2009
        'verifico se i dati della sede che sto inserendo (indirizzo,civico,comune,palazzina,scala,piano,intrno) non sono già prensente nel db
        If ControlloDoppioneSedeNuova(IIf(lblidEnte.Value = 0, Session("IdEnte"), lblidEnte.Value), txtIndirizzo.Text, txtCivico.Text, ddlComune.SelectedValue, Trim(TxtPalazzina.Text), Trim(TxtScala.Text), Trim(TxtPiano.Text), Trim(TxtInterno.Text), "A", txtidsede.Value) = True Then
            cmdSalva.Visible = True
            'Inserisco il Messaggio di Errore
            msgErrore.Text = "Sede già presente in archivio con uguale indirizzo."
            Log.Warning(LogEvent.SEDI_INSERIMENTO_ERRORE, "Sede già presente in archivio con uguale indirizzo.")
    ...
    Private Function ControllaAttivitaSede() As Boolean
        ...
        '
        strsql = "SELECT IDStatoAttività, StatoAttività," & _
                    "(SELECT  COUNT(DISTINCT attività.IDAttività)" & _
                    " FROM attività INNER JOIN " & _
                    " attivitàentisediattuazione ON attività.IDAttività = attivitàentisediattuazione.IDAttività INNER JOIN " & _
                    " entisediattuazioni ON attivitàentisediattuazione.IDEnteSedeAttuazione = entisediattuazioni.IDEnteSedeAttuazione " & _
                    " WHERE (entisediattuazioni.IDEnteSede =" & txtCodice.Value & ") AND attività.idstatoattività = statiattività.idstatoattività AND " & _
                    " attivitàentisediattuazione.numeropostivitto+attivitàentisediattuazione.numeropostivittoalloggio + attivitàentisediattuazione.numeropostinovittonoalloggio<> 0  " & _
                    " and (ATTIVITà.idstatoattività in (4,9) or (ATTIVITà.idstatoattività = 1 and (getdate() between datainizioattività and datafineattività or datainizioattività is null)))) AS nprog " & _
    ...
    Private Sub RipristinaSede()
        ...
        strsql = " update entisedi set idstatoenteSede=4 " & _
        ...
        strsql = " update entisediattuazioni set idstatoenteSede=4 " & _
    ...
    Private Sub AnnullaInclusioneSedeAttuazione()
        ...
        strsql = " Delete  from AssociaEntiRelazioniSediAttuazioni " & _
                 " Where identesedeattuazione in " & _
                 " (Select identesedeattuazione from entisediattuazioni " & _
                 "  where idEnteSede=" & txtidsede.Value & ")"
    ...
    Private Sub AccreditaSede()
        ...
        If ddlTipologia.SelectedItem.Text = "Operativa" Then
            CronologiaStatiEntisediAttuazioni()
        End If
        strsql = " update entisedi set idstatoenteSede =(select idstatoEnteSede from statiEntiSedi where attiva=1) , UsernameStato='" & Session("Utente") & "',DataUltimoStato= GETDATE() " & _
                            " where idEnteSede=" & txtidsede.Value & ""
        If Not dtrGenerico Is Nothing Then
            dtrGenerico.Close()
            dtrGenerico = Nothing
        ...
        If ddlTipologia.SelectedItem.Text = "Operativa" Then
            ...
            strsql = " update entisediAttuazioni set " & _
                     " idstatoenteSede =(select idstatoEnteSede from statiEntiSedi where attiva=1), " & _
                     " Certificazione = 1 ," & _
                     " UserCertificazione ='" & Session("Utente") & "', " & _
                     " DataCertificazione =getdate(), " & _
                     " UsernameStato='" & Session("Utente") & "', DataUltimoStato= GETDATE() " & _
    ...
    Sub CaricaEntiFigli()
        ...
        strsql = strsql & " and  entirelazioni.datafinevalidità is null and enti.idstatoente in (3,6)  order by enti.Denominazione"
...
#Region "Fuzionalita"
    Function dataserver(ByVal data As String) As String
        strsql = "select case len(day(getdate())) when 1 then '0'+ " & _
        " Convert(varchar(20), Day(getdate()))" & _
        " else convert(varchar(20),day(getdate()))  end + '/'+ " & _
        " case len(month(getdate())) when 1 then '0'+ convert(varchar(20),month(getdate())) " & _
        " else convert(varchar(20),month(getdate())) end + '/'+ convert(varchar(20),year(getdate()))" & _
        " as dataserver"
        If Not dtrGenerico Is Nothing Then
            dtrGenerico.Close()
            dtrGenerico = Nothing
    ...
    Private Sub popolamaschera_(ByVal idente As Double, ByVal Ente As String)
        ...
        If lblacquisita.Value <> "propria" Then
            AbilitaCampiMaschera(False)
        Else
            strsql = "Select * from statientisedi where replace(statoentesede, 'Accreditata', 'Iscritta')='" & lblStato.Text & "'"
            If Not dtrGenerico Is Nothing Then
                dtrGenerico.Close()
                dtrGenerico = Nothing
            End If
        ...
        dtrGenerico = ClsServer.CreaDatareader("select day(entisedi.datacontrolloemail)as ggDCEmail,month(entisedi.datacontrolloemail)as " & _
                    ...
                    " entisediTipi.idtiposede,comuni.idComune,comuni.denominazione as Comune,provincie.provincia,* from entisedi " & _
    ...
    Private Sub PersonalizzaTasti()
        ...
        strsql = "Select * from statientisedi where replace(statoentesede, 'Accreditata', 'Iscritta')='" & Trim(Replace(lblStato.Text, "(*)", "")) & "'"
        If Not dtrGenerico Is Nothing Then
            dtrGenerico.Close()
            dtrGenerico = Nothing
        End If
    ...
    Private Sub AbilitaCampiMaschera(ByVal isAbilitato As Boolean)
        ...
        txtLocalita.Enabled = isAbilitato
    ...
    Private Sub RicercaSediAttuazione(ByVal IdEnteSede As Integer)
        ...
        If lblsedeInclusa.Text = "" Then
            ...
            " entisediattuazioni.identeSede,entisediattuazioni.Note,replace(StatoEnteSede, 'Accreditata', 'Iscritta') StatoEnteSede," &
    ...
    Private Function ControllaSedeAttiva() As Boolean
        ...
        For Each item In dgRisultatoRicerca.Items
            If dgRisultatoRicerca.Items(item.ItemIndex).Cells(3).Text = "Iscritta" Or dgRisultatoRicerca.Items(item.ItemIndex).Cells(3).Text = "Presentata" Then
                conta = conta + 1
                Exit For
            End If
        Next
    ...
    Private Sub ControllaSedeFisicaInclusa()
        ...
        strsql = "select associaentirelazionisedi.idassociaentirelazionisedi,entirelazioni.identePadre,entirelazioni.identefiglio,enti.denominazione " & _
            " from " & _
            " associaentirelazionisedi " & _
            " inner join entirelazioni on " & _
            " entirelazioni.identerelazione=associaentirelazionisedi.identerelazione " & _
...
#End Region
    ...
    Private Sub ddlEntiFigli_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
    ...
    Function checkStatoEnteFiglio(ByVal myIntIdEnteFiglio As String) As Boolean
        strsql = "SELECT * FROM entisedi INNER JOIN enti ON entisedi.IDEnte = enti.IDEnte INNER JOIN " & _
                 "statienti ON enti.IDStatoEnte = statienti.IDStatoEnte WHERE statienti.Sospeso = 1 " & _
    ...
    Private Function ControlloSede() As Boolean
        ...
        strsql = "select attività.IdAttività from attivitàEntiSediAttuazione " & _
    ...
    End Function
    Private Function ControlloDoppioneSedeNuova(ByVal IdEnte As Integer, ByVal Indirizzo As String, ByVal Civico As String, ByVal IdComune As Integer, ByVal Palazzina As String, ByVal Scala As String, ByVal Piano As String, ByVal Interno As String, ByVal TipoOperazione As String, Optional ByVal idEnteSede As Integer = 0) As Boolean
        ...
        strSql = strSql & " '" & Scala.Replace("'", "''") & "','" & Piano.Replace("'", "''") & "','" & Interno & "', '" & TipoOperazione & "'," & idEnteSede & ") as DoppioneNuovaSede"
    ...
    Sub PopolaMaschera(ByVal IdEnteSede As Integer)
        ...
        dtrGenerico = ClsServer.CreaDatareader("select day(entisedi.datacontrolloemail)as ggDCEmail,month(entisedi.datacontrolloemail)as " &
                    ...
                    " entisediTipi.idtiposede,comuni.idComune,comuni.denominazione as Comune,provincie.provincia,provincie.idprovincia,provincie.ProvinceNazionali ,replace(StatiEntiSedi.statoentesede, 'Accreditata', 'Iscritta') statoentesede,* from entisedi " &
        ...
        If dtrGenerico.HasRows = True Then
            ...
            Select Case dtrGenerico("IdStatoEnteSede")
                Case 2, 5   ' Cancellata - Richiesta Cancellazione
                    AbilitaCampiMaschera(False)
                    divFuoriAdeguamento.Visible = False
                Case 3      ' Sospesa
    ...
    Private Sub PopolaMascheraVariazione(ByVal IdEnteSede As Integer)
        ...
        dtrGenerico = ClsServer.CreaDatareader("select day(entisedi.datacontrolloemail)as ggDCEmail,month(entisedi.datacontrolloemail)as  " &
                            ...
                            " day(entisedi.datacontrollohttp)as ggDChttp,month(entisedi.datacontrollohttp)as monthDChttp, " &
                            " year(entisedi.datacontrollohttp)as yearDChttp, " &
                            "entisediTipi.idtiposede,comuni.idComune,comuni.denominazione as Comune,provincie.provincia,provincie.idprovincia,provincie.ProvinceNazionali ,replace(StatiEntiSedi.statoentesede, 'Accreditata', 'Iscritta') statoentesede,* 	 " &
    ...
    Private Sub ModificaSedeEnte()
        ...
        Dim idStatoEnteSede As Integer
        
        'leggo Identefase in corso
        Dim managerEnte = New clsEnte()
        Dim strIdEntefase As String = managerEnte.RicavaIdEnteFase(Session("IdEnte"), Session("conn"))
        Dim sqlGetAllegato = "SELECT Top 1 e.idAllegatoLSE, FileName,HashValue,len(BinData) FileLength,BinData,DataInserimento, IdEnteFase, Stato from entidocumenti a inner join entisedi e on a.IdEnteDocumento = e.IdAllegatoLSE where e.identesede=@IdEnteSede"
        Dim AllegatoCommand As New SqlCommand(sqlGetAllegato, Session("conn"))
        AllegatoCommand.Parameters.AddWithValue("@idEnteSede", CInt(Session("idEnteSede")))
        ...
        If Session("LoadLSEId") = Nothing And ChkEstero.Checked And ddlTitoloGiuridico.SelectedValue = 7 Then
            ...
            If LSE.Hash <> hashValue Then
                If idAllegatoOld = String.Empty Or lblStato.Text.ToUpper.Trim = "ISCRITTA" Then
                    Dim idAllegato As Integer = SalvaAllegato(LSE, TipoFile.LETTERA_SEDE_ESTERA, strIdEntefase)
                    LSE.Id = idAllegato
                    Session("LoadLSEId") = idAllegato
                    Log.Information(LogEvent.SEDI_IMPORTAZIONE_LSE)
                Else
                    If CercaAllegato(LSE, strIdEntefase) Then
                        AggiornaAllegato(LSE, strIdEntefase)
        ...
        Try
            SqlCmd.CommandText = "SP_ACCREDITAMENTO_MODIFICAMASCHERA_SEDE"
            SqlCmd.CommandType = CommandType.StoredProcedure
            SqlCmd.Connection = Session("Conn")
            ...
            If ddlComune.SelectedValue = 0 Then
                SqlCmd.Parameters.Add("@IdComune", SqlDbType.Int).Value = DBNull.Value
            Else
            ...
            'se indico un titolo giuridico =ALTRO specifico nella text il titolo giuridico
            If ddlTitoloGiuridico.SelectedItem.Text <> "Altro" Then
                SqlCmd.Parameters.Add("@AltroTitoloGiuridico", SqlDbType.VarChar).Value = DBNull.Value
            Else
                SqlCmd.Parameters.Add("@AltroTitoloGiuridico", SqlDbType.VarChar).Value = txtAltroTitoloGiuridicio.Text
            End If
            ...
            Else
                ...
                SqlCmd.Parameters.Add("@DataControlloHttp", SqlDbType.DateTime).Value = lblDataControlloHttp.Value 'DataControlloHttp
            ...
            Else
                ...
                SqlCmd.Parameters.Add("@DataControlloEmail", SqlDbType.DateTime).Value = lbldataControlloEmail.Value
            ...
            If txtNote.Text = "" Then
                SqlCmd.Parameters.Add("@Note", SqlDbType.VarChar).Value = DBNull.Value
            Else
                SqlCmd.Parameters.Add("@Note", SqlDbType.VarChar).Value = txtNote.Text
            End If
            If ChkEstero.Checked = True Then
            ...
            SqlCmd.Parameters.Add("@Certificazione", SqlDbType.TinyInt).Value = ddlCertificazione.SelectedValue
            SqlCmd.Parameters.Add("@RipristinoSegnalazione", SqlDbType.VarChar).Value = UCase(txtRipristinoSanzione.Value)
            ...
            SqlCmd.ExecuteNonQuery()
            Dim messaggio As String = SqlCmd.Parameters("@messaggio").Value
            Dim esito As Integer = SqlCmd.Parameters("@Esito").Value
            ...
            Dim ckAllegato As Boolean = False
            If Session("LoadLSEId") Is Nothing And idAllegatoOld <> String.Empty Then
                ckAllegato = True
            End If
            If ddlTitoloGiuridico.SelectedValue <> 7 And idAllegatoOld <> String.Empty Then
                ckAllegato = True
            End If

            If ckAllegato = True Then
                Dim sqlDelAllegato = "Delete from  ALLEGATO where IdAllegato = @IdAllegato"
                Dim DelAllegatoCommand As New SqlCommand(sqlDelAllegato, Session("conn"))
                DelAllegatoCommand.Parameters.AddWithValue("@IdAllegato", CInt(idAllegatoOld))
                DelAllegatoCommand.ExecuteNonQuery()
                DelAllegatoCommand.Dispose()

                Log.Information(LogEvent.SEDI_IMPORTAZIONE_LSE, "Cancellato Allegato con Id: " & idAllegatoOld)
            End If
    ...
    Private Sub cmdAnnullaModifica_Click(ByVal sender As Object, ByVal e As EventArgs) Handles cmdAnnullaModifica.Click
        ...
        AnnullaModificaDocumenti(txtCodice.Value)
    ...
    Private Sub LoadMaschera(ByVal IdEnteSede As Integer)
        ...
        strsql = " SELECT IDENTE,IDSTATOENTE,CODICEREGIONE,isnull(flagforzaturaaccreditamento ,0) FLGABILITAZIONE FROM ENTI WHERE IDENTE = @IdEnte "
        ...
        If Request.QueryString("tipoazione") <> "Inserimento" Then
            ...
            'verifico se è possibile la modifica dati fuori adeguamento
        ...
        If abilitato = 0 Or abilitato = 2 Then ' -- 0: maschera sola lettura 1: maschera in modifica 2: sola cancellazione
...
#Region "Gestione indirizzi"
    ...
    Private Sub ddlProvincia_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlProvincia.SelectedIndexChanged
        ...
        ddlComune.ClearSelection()
        If ChkEstero.Checked And ddlProvincia.SelectedValue = True Then
            ddlComune.SelectedIndex = 1
        Else
            ddlComune.SelectedValue = 0
        End If
    End Sub
...
#End Region
    ...
    Private Function VerificaValiditaCampi(ByVal AlboEnte As String) As Boolean
        ...
        If ChkEstero.Checked = False And ddlComune.SelectedValue <> 0 Then
            ...
            'If AlboEnte = "SCU" Then
            '    If TitoloGiuridico_ObbligoScandeza(ddlTitoloGiuridico.SelectedValue) = True Then

            '        If (txtDataStipulaContratto.Text <> String.Empty) Then
            '            If (Date.TryParse(txtDataStipulaContratto.Text, dataTmp) = False) Then
            '                msgErrore.Text = msgErrore.Text + String.Format(messaggioDataValida, "Data Stipula Contratto")
            '                campiValidi = False
            '            End If
            '        Else
            '            msgErrore.Text = msgErrore.Text + String.Format(campoObbligatorio, "Data Stipula Contratto")
            '            campiValidi = False
            '        End If
            '        If (txtDataScadenzaContratto.Text <> String.Empty) Then
            '            If (Date.TryParse(txtDataScadenzaContratto.Text, dataTmp) = False) Then
            '                msgErrore.Text = msgErrore.Text + String.Format(messaggioDataValida, "Data Scadenza Contratto")
            '                campiValidi = False
            '            End If
            '        Else
            '            msgErrore.Text = msgErrore.Text + String.Format(campoObbligatorio, "Data Scadenza Contratto")
            '            campiValidi = False
            '        End If
            '        If (txtDataScadenzaContratto.Text <> String.Empty) And (txtDataStipulaContratto.Text <> String.Empty) Then
            '            If CDate(txtDataStipulaContratto.Text) > CDate(txtDataScadenzaContratto.Text) Then
            '                msgErrore.Text = msgErrore.Text + "La Data Scadenza Contratto deve essere successiva alla Data Stipula."
            '                campiValidi = False
            '            End If
            '        End If
            '    End If
    ...
    Sub EvidenziaDatiModificati(ByVal IdEnteSede As Integer)
        ...
        strsql = " SELECT comuni.idComune,comuni.denominazione as Comune,provincie.provincia,replace(StatiEntiSedi.statoentesede, 'Accreditata', 'Iscritta') statoentesede," &
            ...
            If esito = 4 Or (Session("ProcediGM") = 0 ) Then
                ...
                If b = False Then
                    ...
                    ElseIf (indirizzoErratoHelios = False And (strMiaCausale.ToUpper.Trim = "CAP ESISTENTE PER COMUNE SENZA INDIRIZZI" _
                            Or strMiaCausale.ToUpper.Trim = "COMUNE ESTERO" Or strMiaCausale.ToUpper.Trim = "COMUNE NON TROVATO")) Or indirizzoErratoHelios = True Then
                        lblGeolocalizzazione.Text = ""
                        lblGeolocalizzazione.Visible = False
    ...
    Private Sub CaricaLSE()
        ...
        If Session("LoadLSEId") IsNot Nothing Then
            CaricaAllegatoFromDB(Session("LoadLSEId"), "LoadedLSE", rowNoLSE, rowLSE, txtLSEFilename, txtLSEHash, txtLSEData)
        Else
            ClearSessionLSE()
        End If
    ...
    Private Function getRegioneCompetenzaEnte() As Integer
        ...
        'MV Modificato 12/08/2021
        'Se si è in adeguamento bisogna dare precedenza all'idRegioneCompetenza (che può essere stato modificato) in Accreditamento_VariazioneEnti
        Dim strSql = "select coalesce(t4.IdRegioneCompetenza,t2.idRegioneCompetenza) idRegioneCompetenza"
        strSql += " from enti t1 "
        strSql += " inner join sezionialboscu t2 on t2.idSezione = t1.idSezione"
        strSql += " left join Accreditamento_VariazioneEnti t3 on t3.IdEnte=t1.IDEnte and t3.StatoVariazione=0"
        strSql += " left join sezionialboscu t4 on t4.idSezione = t3.idSezione"
        strSql += " where t1.idEnte=" & Session("IdEnte")
    ...
    Private Sub cmdFuoriAdeguamento_Click(sender As Object, e As EventArgs) Handles cmdFuoriAdeguamento.Click
        Response.Redirect("WfrmAggiornaSediFuoriAdeguamento.aspx?identesede=" & Session("IdEnteSede") & "&Idente=" & lblidEnte.Value)
    End Sub

    Sub verificaNumeroSedi()
        Dim EnteOriginale As Integer
        ...
        If ReaderSedi.Read Then
            If CInt(ReaderSedi("NSedi")) < 2 Then
                msgErrore.Text = msgErrore.Text + "ATTENZIONE!!!: E' stata rimossa l'unica sede di attuazione dell'ente."
    ...
    Sub AnnullaModificaDocumenti(IdEnteSede As String)
        'VA CHIAMATA PRIMA DELL'ANNULLAMENTO RECORD ACCREDITAMENTO
        Dim SqlCmd As New SqlCommand
        SqlCmd.CommandText = "SP_ACCREDITAMENTO_ANNULLA_MODIFICA_DOCUMENTI_SEDE"
        SqlCmd.CommandType = CommandType.StoredProcedure
        SqlCmd.Connection = Session("Conn")
