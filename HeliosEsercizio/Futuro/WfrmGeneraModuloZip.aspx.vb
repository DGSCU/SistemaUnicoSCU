...
Imports Ionic.Zip
Public Class WfrmGeneraModuloZip
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Page.IsPostBack = False Then
            CaricaCombo()
            If Request.QueryString("VengoDa") = "GeneraModuloZip" Then
                LoadRiceraVolontari()
            End If
        End If
    ...
    Protected Sub dgVolontari_SelectedIndexChanged(sender As Object, e As EventArgs) Handles dgVolontari.SelectedIndexChanged
        ...
        Response.Redirect("Presenze.aspx?IdEntita=" & dgVolontari.SelectedItem.Cells(0).Text & "&anno=" & stranno & "&mese=" & strmese & "&AnnoMese=" & CboAnnoMese.SelectedValue & "&VengoDa=GeneraModuloZip")
    ...
    Protected Sub CmdGenaraModuloZip_Click(sender As Object, e As EventArgs) Handles CmdGenaraModuloZip.Click
        Dim strMg As String = ""
        Dim esito As String
        Dim myfile As Byte()
        ...
        Try
            ...
            For Each riga As DataGridItem In dgVolontari.Items
                If CType(riga.FindControl("chkSelProg"), CheckBox).Checked Then
                    ...
                    strMg = ""
                    myfile = ClsServer.GeneraModuloPresenze(riga.Cells(0).Text, Left(CboAnnoMese.SelectedValue, 4), Right(CboAnnoMese.SelectedValue, 2), Server.MapPath("download\Master\Volontari\ModuloPresenze.docx"), Session("Utente"), Session("conn"), strMg)

                    If strMg = "" Then
                        NomeFilePdf = riga.Cells(4).Text & "_" & Left(CboAnnoMese.SelectedValue, 4) & "_" & Right(CboAnnoMese.SelectedValue, 2) & ".pdf"
                        PathCompleto = Server.MapPath("download") & "\" & NomeFilePdf
                        FileByteToPath(myfile, NomeFilePdf)

                        zip.AddFile(PathCompleto, "\")



                        'ClsServer.ConfermaPreliminare(riga.Cells(0).Text, Left(CboAnnoMese.SelectedValue, 4), Right(CboAnnoMese.SelectedValue, 2), Session("Utente"), Session("conn"), strMg, esito)
            ...
            Else
                If intNAggiornati = intNSelezionati Then
                    lblmessaggio.Text = "Generazione Modulo Zip effettuata."
                Else
                    lblmessaggio.Text = "Sono state generati " & intNAggiornati & " file su " & intNSelezionati & ". E' possibile generare i moduli precompilati solo per i mesi con conferma preliminare effettuata. Procedere accedento ai singoli calendari per verificare le eventuali eccezioni."
                End If
            End If

            If intNAggiornati > 0 Then
                'trovare nome per file zip!!!!!!!!!!!!!
                Dim NomeUnivoco As String
                NomeUnivoco = Session("Utente") & "_MODULI_PRESENZE_" & Year(Now) & Month(Now) & Day(Now) & Hour(Now) & Minute(Now) & Second(Now) & ".zip"

                zip.Save(Server.MapPath("download") & "\" & NomeUnivoco)


                Response.Clear()
                Response.ContentType = "Application/zip"
                Response.AddHeader("Content-Disposition", "attachment; filename=" & NomeUnivoco)
                Response.WriteFile(Server.MapPath("download") & "\" & NomeUnivoco)
                Response.End()

                'cancellare tutti i pdf scaricati!!!!!!!
                'If File.Exists(PathCompleto) Then
