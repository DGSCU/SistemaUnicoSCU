...
Imports System.IO
Public Class WfrmProgrammidaValutare
...
#End Region
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        End If
        LblStatoValutazione.Visible = False
        ddlStatoValutazione.Visible = False
        lblCboCompetenza.Visible = False
        CboCompetenza.Visible = False
    End Sub
    ...
    Private Sub CaricaDatiIniziali()
        ...
        ddlStatoAttivita.DataSource = MakeParentTable("select idstatoprogramma, statoprogramma from statiprogrammi")
        ...
        If Request.QueryString("VengoDa") = "Valutare" Then
            dtrgenerico = ClsServer.CreaDatareader("select idstatoprogramma from statiprogrammi where daValutare=1", IIf(ClsServer.Connessione(Session("TipoUtente"), Session("IdStatoEnte"), Session("ConnSnapshot"), Session("conn"), Session("FlagForzatura")) = False, Session("ConnSnapshot"), Session("Conn")))
    ...
    Private Sub EseguiRicerca(ByVal bytVerifica As Byte, Optional ByVal bytpage As Integer = 0)
        ...
        'If Request.QueryString("VengoDa") = "Valutare" Then
        '    strquery = "select  distinct b.CodiceRegione, b.denominazione, a.titolo, c.bando,c.bandobreve," & _
        ...
        '               " IsNull(NumeroPostiNoVittoNoAlloggio,0) + IsNull(NumeroPostiVittoAlloggio,0) + IsNull(NumeroPostiVitto,0) As Effettivi, " & _
        '               " IsNull(NumeroPostiNoVittoNoAlloggioRic,0) + IsNull(NumeroPostiVittoAlloggioRic,0) + IsNull(NumeroPostiVittoRic,0) As Richiesti,RegioniCompetenze.Descrizione as Competenza, " & _
        ...
        strquery = "select distinct e.CodiceRegione, e.Denominazione, a.Titolo, a.CodiceProgramma, c.Bando, c.BandoBreve, dbo.formatodata(a.datapresentazioneprogramma) as Data,a.idprogramma, e.IDEnte, a.IdTipoProgramma,  IsNull(a.NumeroPostiNoVittoNoAlloggio,0) + IsNull(a.NumeroPostiVittoAlloggio,0) + IsNull(a.NumeroPostiVitto,0) As Effettivi,  IsNull(a.NumeroPostiNoVittoNoAlloggioRic,0) + IsNull(a.NumeroPostiVittoAlloggioRic,0) + IsNull(a.NumeroPostiVittoRic,0) As Richiesti" & _
                " from programmi a" & _
                ...
                " inner join attività f on a.IdProgramma = f.IdProgramma" & _
                " inner join TipiProgetto g on f.IdTipoProgetto = g.IdTipoProgetto" & _
                " INNER JOIN AssociaProfiliTipiProgetto ON g.IdTipoProgetto = AssociaProfiliTipiProgetto.IdTipoProgetto" & _
                " INNER JOIN Profili ON AssociaProfiliTipiProgetto.IdProfilo = Profili.IdProfilo"
        ...
        strquery = strquery & " where d.InValutazione = 1 and AssociaUtenteGruppo.username='" & Replace(Session("Utente"), "'", "''") & "'"


        If ddlStatoAttivita.SelectedItem.Text <> "" Then
            strquery = strquery & " and a.idstatoprogramma=" & ddlStatoAttivita.SelectedValue & ""
        ...
        If txtDenominazioneEnte.Text <> "" Then
            strquery = strquery & " and e.Denominazione like '" & ClsServer.NoApice(Trim(txtDenominazioneEnte.Text)) & "%'"
        End If
        ...
        'End If
        strquery = strquery & " AND g.MacroTipoProgetto like '" & Session("FiltroVisibilita") & "'"
        strquery = strquery & " order by 1,3"
        ...
        'dgRisultatoRicerca.Columns(15).Visible = ClsUtility.ForzaPresenzaSanzioneProgetto(Session("Utente"), Session("conn"))
        ...
        Else
            CmdEsporta.Visible = True
            dgRisultatoRicerca.Caption = "Risultato Ricerca Programmi"
        End If
    ...
    Sub CaricaDataTablePerStampa(ByVal DataSetDaScorrere As DataSet)
        ...
        NomeColonne(3) = "Bando"
        NomeColonne(4) = "Data presentazione"
        NomeColonne(5) = "Posti Effettivi"
        NomeColonne(6) = "Posti Richiesti"
        ...
        NomiCampiColonne(5) = "Effettivi"
        NomiCampiColonne(6) = "Richiesti"
    ...
    Private Sub dgRisultatoRicerca_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles dgRisultatoRicerca.ItemCommand
        ...
        Select Case e.CommandName
            Case "Select"
                ...
                strquery = "select isnull(b.idstatobandoprogramma,0) as idstatobandoprogramma from programmi a left join bandiprogrammi b on a.idbandoprogramma = b.idbandoprogramma " & _
                    " where a.idprogramma = " & CInt(e.Item.Cells(8).Text) & ""
                ...
                If dtrGenerico("idstatobandoprogramma") = 3 Then
                    ...
                    If (Session("TipoUtente") = "U" Or Session("TipoUtente") = "R") Then
                        Session("IdEnte") = e.Item.Cells(9).Text
                        Session("Denominazione") = e.Item.Cells(3).Text
                    End If
                    Response.Redirect("assegnazionevincoliprogrammi.aspx?idprogramma=" & CInt(e.Item.Cells(8).Text) & "&tipologia=ProgrammiValutare")
                Else
                    'Messaggio
                    msgErrore.Text = "L'istanza non risulta essere approvata."
                    ChiudiDataReader(dtrGenerico)
                    Exit Sub
                End If
        End Select
